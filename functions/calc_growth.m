...
function TM_SGR = calc_growth(TM_conc, dates, phase, TM_growth)
    ...
    %Comment if data inputted as a long matrix, instead of one with pages.
    %transpose for reshaping
    TM_growth = TM_growth';
    %reshape column wise into a wide matrix 
    TM_growth_tr = reshape(TM_growth_pg_tr, 5, nr_trt*length(TM_growth(:,2,1))); 
    %transpose into a long matrix
    TM_growth = TM_growth_tr'; 

    %Do a check for when running the script and having not specified the
    %selected mid exp data points

    if phase == 0
        disp('Cannot calculate SGR, specify phase');
